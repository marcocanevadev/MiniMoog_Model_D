...
class MiniMoog(PyoObject):
    ...
    def __init__(self ):
        ...
        self._osc2_oct = Sig(1)
        self._osc3_oct = Sig(1)

        self._osc1_wf =Sig(0)
        self._osc2_wf =Sig(0)
        self._osc3_wf =Sig(0)

        self._t_1 = LinTable([(0,-0.5),(3472,-0.3),(3480,1),(4096,1),(5120,1),(5128,-0.7),(8191,-0.5)])
        self._t_2 = LinTable([(0,-0.5),(3600,-0.3),(3608,1),(4096,1),(4990,1),(4998,-0.7),(8191,-0.5)])
        self._list_1 = [
            RCOsc((self._notes["pitch"]*4)/(2**(5-self._osc1_oct)),mul = self._env, sharp=0),
            Sharktooth((self._notes["pitch"]*4)/(2**(5-self._osc1_oct)),mul = self._env*self._osc1_switch),
            LFO((self._notes["pitch"]*4)/(2**(5-self._osc1_oct)),mul = self._env*self._osc1_switch, sharp=1,type = 0),
            RCOsc((self._notes["pitch"]*4)/(2**(5-self._osc1_oct)),mul = self._env*self._osc1_switch, sharp=1),
            Osc(self._t_1, freq = (self._notes["pitch"]*4)/(2**(5-self._osc1_oct)),mul = self._env*self._osc1_switch),
            Osc(self._t_2, freq = (self._notes["pitch"]*4)/(2**(5-self._osc1_oct)),mul = self._env*self._osc1_switch)
        ]

        self._list_2 = self._list_1
        self._list_2[1] = LFO((self._notes["pitch"]*4)/(2**(5-self._osc1_oct)),mul = self._env*self._osc1_switch, sharp=1,type = 0)
        print(self._list_2)
        print(self._list_1)
        self._osc1 = Selector(self._list_1,self._osc1_wf)
        self._osc1._mul *= self._osc1_switch
        ### DISCARDED self._oct1 = self._octaves[self._osc1_oct.value]

        #self._osc1= LFO((self._notes["pitch"]*4)/(2**(5-self._osc1_oct)),mul = self._env*self._osc1_switch, sharp=1)
        self._osc2= Selector(self._list_1,self._osc2_wf)
        self._osc3= LFO((self._notes["pitch"]*4)/(2**(5-self._osc3_oct)), mul = self._env*self._osc3_switch, sharp=1)
       ...
       # self._LFO_freq = Sig(0)
       # self._LFO_mul= Sig(0.5)
       # self._LFO_select = Sig(3)
        ...
        self._LP = MoogLP(self._mix)
        self._out= Pan(self._LP*self._LFO)
